...
JB.Gamemode.PlayerShouldTakeDamage = function(gm,a,b)
	if IsValid(a) and IsValid(b) and a:IsPlayer() and b:IsPlayer() and a:Team() == b:Team() and (JB.State == STATE_SETUP or JB.State == STATE_PLAYING or JB.State == STATE_LASTREQUEST) and (not IsValid(JB.TRANSMITTER) or a:Team() ~= TEAM_PRISONER or not JB.TRANSMITTER:GetJBWarden_PVPDamage()) or (not IsValid(JB.TRANSMITTER) or a:Team() ~= TEAM_GUARD or not JB.TRANSMITTER:GetJBWarden_PVPDamageGuards()) then
		return false
	end
	return true;
end
